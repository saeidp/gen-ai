...
import rag_backend_cohere_claude3 as backend ### replace rag_backend with your backend filename
...
if 'vector_index' not in st.session_state: 
    with st.spinner("📀 Wait for magic...All beautiful things in life take time :-)"): ###spinner message
        st.session_state.vector_index = backend.hr_index() ### Your Index Function name from Backend File
...
if go_button: 
    ...
    with st.spinner("📢Anytime someone tells me that I can't do something, I want to do it more - Taylor Swift"): ### Spinner message
        response_content = backend.hr_rag_response(index=st.session_state.vector_index, question=input_text) ### replace with RAG Function from backend file
